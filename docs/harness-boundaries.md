...
issue in THAT repository, not one here** — chase it there or not at all.
And where a paragraph below argues from something measured over there — an
`Exec` in an auto-imported MSBuild file, a grant withdrawn from a command
this repository does not have — the measurement stands and the toolchain in
it does not; the same paragraph names this repository's equivalent.

**The content is otherwise verbatim in its arguments**, and for the same
reason it was when it was extracted from `CLAUDE.md` there. Four edits were made on the
...
> **Nothing structural catches this file's drift.** The two commands that
> audited documents over there — `/validate-blueprint` and `/check-links` —
> read a blueprint tree this repository does not have and were not ported, so
> no command here reads this file at all. The one rule in `CLAUDE.md` covers
> it, and that is all that does. That is a gap rather than a decision, and it
> is the reason the paragraph below insists the description beats a list.
>
...
>
> **A predicate is only better than a list if it is true.** These callouts
> first said this file is in no index “like every document under `docs/`
> that is not the blueprint tree”, which was false twice over in the
> repository that wrote it. Scope is the durable fact and being unindexed
> never was — a false predicate is a stale list with an argument in front of
> it.
...
claim about it. The
reviewer also has **no Node toolchain**, and could not use one: every gate
here starts with `npm ci`, which needs `registry.npmjs.org`, and the egress
allow-list admits two hosts and neither is it. So lint, the unit suite and
the build are this host's gates and never the review's.
...
**The ninth is `/review-branch`'s build grant, and what makes it worth its own
entry is that the enumeration guarding it could never have worked.** That
command holds `Write`; its `disallowed-tools` denies every tracked root file,
read from `git ls-files` by a test so a new one is a red build. But a build
reads files nobody asked it to, and the dangerous one is the file that does
**not** exist yet — so it is on no list read from what exists, and no care
about that test could have put it there. Write it, run the build the command
already had, and it executes. Measured over there, in a scratch tree: an
`Exec` in an auto-imported MSBuild targets file fires and the build reports
success. Raised in review.

**The same shape is sharper in this toolchain, not milder.** `eslint.config.js`
and a `vitest.config.ts` are JavaScript that is executed in order to be
loaded; `.npmrc` can set `script-shell`; a `pre`/`post` entry in
`package.json` runs on every `npm` invocation. And the grant that would have
bought it — `Bash(npm run:*)` — admits any script the manifest names,
including one the command has just written, while `npm test -- --config
<path>` admits a file Vitest executes to read, `suggestions.md` among them.
None of those is granted: the executor is `npm-checks.sh`, whose only
variable is one word out of a closed set, and `test_grok_helpers.py` asserts
that no command's frontmatter carries `Bash(npm `, `Bash(npx `, `Bash(node `,
`Bash(ng `, `Bash(yarn ` or `Bash(pnpm `.
**The helper closes the executor and not the manifest**, so the auto-load
names are denied as well, in the one command that states editing boundaries
— the artefact outlives the command that writes it.

**Which half is measured is recorded rather than rounded up.** The exact
filenames are the spelling the file already used and the suite reads. The
`**/*.config.js`-style globs beside them are the documented gitignore syntax
and are **not** measured in a `disallowed-tools` value — belt to the names' braces,
and not the control.
...
audits**, because building a tree executes it — `npm ci` runs any
...
Vitest config are executed in order to be loaded, and `npm test` runs the
tree's own test code — and the audited repository is prompt-injection input. So a defect claim there is
