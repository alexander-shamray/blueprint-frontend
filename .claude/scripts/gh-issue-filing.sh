...
# in prose that nothing can filter on. So each route ends the body with the
...
#
# THE ROUTE IS NOT A PARAMETER ANY MORE, AND #19 IS WHY. It was the one
# argument that escaped the reasoning three paragraphs up. Both sweeps held
# `Bash(bash .claude/scripts/gh-issue-create.sh:*)`, a prefix grant, so the
# model chose all three arguments — including which provenance claim the issue
# carried. The dangerous direction is a hand-filed or injected finding claiming
# `sweep`, which asserts a verification that did not happen: the same shape as
# a gate that passes vacuously, in the sentence a triager reads to decide
# whether anybody checked.
#
# So this file is a library and the route is a constant in each of the two
# entry points that source it — `gh-issue-create.sh` files `hand` and
# `gh-sweep-issue-create.sh` files `sweep`, neither able to name the other's
# trailer because neither has an argument for it. The sweeps grant the second
# and deny the first by name; the hand path grants nothing and needs nothing.
#
# **The residual, because a split is not a proof of provenance.** Nothing stops
# a session invoking the sweep entry point by hand — no `gh` rule exists at all
# and this is a `bash` invocation like any other. What the split removes is the
# case that runs UNATTENDED over a tree that is prompt-injection input, which
# is the one where nobody is reading the argument as it is chosen.
set -euo pipefail

# Sourced, never run. The two entry points beside this file each pass a
# constant route; `$0` is still the entry point's name, so a usage line printed
# from in here names the command the caller actually typed.
gh_issue_file() {
  local kind="$1" severity="$2" route="$3"
  local trailer title separator body last repo here
