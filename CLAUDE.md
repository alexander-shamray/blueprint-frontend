...
| [`.mcp.json`](.mcp.json) | Local index as an MCP server (`--root .`). Install the CLI first: `py -3.12 -m pip install codebase-index` (or `python -m pip`); it is not a repo package |
